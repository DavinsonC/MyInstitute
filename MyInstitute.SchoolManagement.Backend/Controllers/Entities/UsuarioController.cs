...
using MyInstitute.SchoolManagement.Backend.Helpers;
...
using MyInstitute.SchoolManagement.Shared.Pagination;
using MyInstitute.SchoolManagement.Shared.Responses;
using System.Security.Claims;

namespace MyInstitute.SchoolManagement.Backend.Controllers.Entities
{
    [Route("api/Usuarios")]
    [ApiController]
    public class UsuarioController : ControllerBase
    {
        ...
        public UsuarioController(DataContext context, IFileStorage fileStorage,
        ...
        public async Task<ActionResult<IEnumerable<Usuario>>> GetUsuarios([FromQuery] PaginationDTO pagination)
        {
            ...
            return await queryable.OrderBy(x => x.FirstName).Paginate(pagination).ToListAsync();
        }
        // GET BY ID:
        [HttpGet("{id}")]
        ...
        public async Task<ActionResult<Usuario>> PostUsuario(Usuario modelo)
        {
            try
            {
                ...
                await transaction.CommitAsync();
                return CreatedAtAction("GetUsuario", new { id = modelo.UsuarioId }, modelo);
            }
            catch (DbUpdateException dbUpdateException)
            {
                if (dbUpdateException.InnerException!.Message.Contains("duplicate"))
        ...
        private async Task<Response> AcivateUser(Usuario manager)
        {
            User user = await _userHelper.AddUserUsuarioSoftAsync(manager.FirstName, manager.LastName, manager.UserName,
                manager.PhoneNumber!, manager.Address!, manager.Job!, manager.UsuarioId, manager.Photo!, "UsuarioSoftware", manager.Active);
